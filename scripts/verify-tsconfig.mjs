...
const rootTsconfig = JSON.parse(
  await fs.readFile('./tsconfig.json', { encoding: 'utf8' }),
);

const rootTsconfigReferences = new Set(
  rootTsconfig.references.map(({ path }) => path.split('/').pop()),
);
...
const unreferencedPackageDirs = packageDirNames.filter(
  (name) => !rootTsconfigReferences.has(name) && !IGNORE_LIST.has(name),
);

if (unreferencedPackageDirs.length > 0) {
  throw new Error(
    `Found unreferenced package directories not in ignore list:\n\n\t${unreferencedPackageDirs.join(
      '\n\t',
