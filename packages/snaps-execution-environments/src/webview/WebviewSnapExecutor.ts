...
export class WebviewSnapExecutor {
  ...
  /**
   * Terminate the job with the given ID. This will close the iframe, delete
   * the job from the internal job map, and destroy the job's stream using destroy function internally
   * to mark the stream instance as destroyed and calls the close.
   *
   * @param jobId - The job ID.
   */
