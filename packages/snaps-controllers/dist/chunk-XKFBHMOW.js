...
var SnapController = class extends _basecontroller.BaseController {
  constructor({
    ...
    featureFlags = {},
    detectSnapLocation: detectSnapLocationFunction = _chunkPOQSARDGjs.detectSnapLocation,
    ...
    _chunkEXN2TFDJjs.__privateAdd.call(void 0, this, _isSubjectConnectedToSnap);
    _chunkEXN2TFDJjs.__privateAdd.call(void 0, this, _calculateConnectionsChange);
    /**
  ...
  async getSnapFile(snapId, path, encoding = _snapssdk.AuxiliaryFileEncoding.Base64) {
    ...
    const encoded = await _snapsutils.encodeAuxiliaryFile.call(void 0, value, encoding);
    _utils.assert.call(void 0, 
      encoded.length < _snapsutils.MAX_FILE_SIZE,
  ...
  async updateSnap(origin, snapId, location, newVersionRange = _snapsutils.DEFAULT_REQUESTED_SNAP_VERSION, emitEvent = true) {
    ...
    try {
      ...
      const newSnap = await _chunkB6KTRDBKjs.fetchSnap.call(void 0, snapId, location);
      ...
      const { newConnections, unusedConnections, approvedConnections } = _chunkEXN2TFDJjs.__privateMethod.call(void 0, this, _calculateConnectionsChange, calculateConnectionsChange_fn).call(this, snapId, oldManifest.initialConnections ?? {}, manifest.initialConnections ?? {});
      _chunkEXN2TFDJjs.__privateMethod.call(void 0, this, _updateApproval, updateApproval_fn).call(this, pendingApproval.id, {
...
handleInitialConnections_fn = function(snapId, previousInitialConnections, initialConnections) {
  if (previousInitialConnections) {
    const revokedInitialConnections = _chunkB6KTRDBKjs.setDiff.call(void 0, 
...
add_fn = async function(args) {
  ...
  if (!runtime.installPromise) {
    ...
    runtime.installPromise = (async () => {
      const fetchedSnap = await _chunkB6KTRDBKjs.fetchSnap.call(void 0, snapId, location);
...
getRpcRequestHandler_fn = function(snapId) {
  ...
  const rpcHandler = async ({
    ...
    try {
      const result = await _chunkB6KTRDBKjs.withTimeout.call(void 0, handleRpcRequestPromise, timer);
      if (result === _chunkB6KTRDBKjs.hasTimedOut) {
        throw new Error(
...
calculatePermissionsChange_fn = function(snapId, desiredPermissionsSet) {
  ...
  const newPermissions = _chunkB6KTRDBKjs.permissionsDiff.call(void 0, 
    desiredPermissionsSet,
    oldPermissions
  );
  const unusedPermissions = _chunkB6KTRDBKjs.permissionsDiff.call(void 0, 
    oldPermissions,
    desiredPermissionsSet
  );
  const approvedPermissions = _chunkB6KTRDBKjs.permissionsDiff.call(void 0, 
    oldPermissions,
    unusedPermissions
  );
...
calculateConnectionsChange_fn = function(snapId, oldConnectionsSet, desiredConnectionsSet) {
  const filteredOldConnections = Object.keys(oldConnectionsSet).filter((origin) => _chunkEXN2TFDJjs.__privateMethod.call(void 0, this, _isSubjectConnectedToSnap, isSubjectConnectedToSnap_fn).call(this, snapId, origin)).reduce((accumulator, origin) => {
    accumulator[origin] = oldConnectionsSet[origin];
    return accumulator;
  }, {});
  const newConnections = _chunkB6KTRDBKjs.setDiff.call(void 0, 
    desiredConnectionsSet,
    filteredOldConnections
  );
  const unusedConnections = _chunkB6KTRDBKjs.setDiff.call(void 0, 
    filteredOldConnections,
    desiredConnectionsSet
  );
  const approvedConnections = _chunkB6KTRDBKjs.setDiff.call(void 0, 
    filteredOldConnections,
    unusedConnections
  );
...
//# sourceMappingURL=chunk-XKFBHMOW.js.map