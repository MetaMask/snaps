...
const { WorkerPostMessageStream } = require('@mm-snap/post-message-stream');
...
function connectToParent() {
  ...
  const backgroundApiStream = mux.createStream(
    PLUGIN_STREAM_NAMES.BACKGROUND_API,
  );
...
function _onCommandMessage(message) {
  ...
  switch (command) {
    ...
    case 'pluginRpc':
      ...
      self
        .pluginRpcHandler(data.origin, data.request)
        .then((result) => {
          respond({ result });
        })
        .catch((error) => {
